...
public class TestTrenchBroomExpressions
{
    ...
    public void TestTypeConversions()
    {
        ...
        Assert.Throws<InvalidCastException>(() => new Value(true).ConvertTo(DataType.Array));
        Assert.Throws<InvalidCastException>(() => new Value(false).ConvertTo(DataType.Array));
        Assert.Throws<InvalidCastException>(() => new Value(true).ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => new Value(false).ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => new Value(true).ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => new Value(false).ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => new Value(true).ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => new Value(false).ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => new Value(true).ConvertTo(DataType.Undefined));
        Assert.Throws<InvalidCastException>(() => new Value(false).ConvertTo(DataType.Undefined));
        ...
        Assert.Throws<InvalidCastException>(() => new Value("asdf").ConvertTo(DataType.Number));
        Assert.Throws<InvalidCastException>(() => new Value("asdf").ConvertTo(DataType.Array));
        Assert.Throws<InvalidCastException>(() => new Value("asfd").ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => new Value("asdf").ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => new Value("asdf").ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => new Value("asdf").ConvertTo(DataType.Undefined));
        ...
        Assert.AreEqual(new Value(-1), new Value(-1.0).ConvertTo(DataType.Number));
        Assert.Throws<InvalidCastException>(() => new Value(1).ConvertTo(DataType.Array));
        Assert.Throws<InvalidCastException>(() => new Value(2).ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => new Value(3).ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => new Value(4).ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => new Value(5).ConvertTo(DataType.Undefined));

        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.Boolean));
        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.String));
        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.Number));
        Assert.AreEqual(new Value(arrayType), new Value(arrayType).ConvertTo(DataType.Array));
        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => new Value(arrayType).ConvertTo(DataType.Undefined));

        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.Boolean));
        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.String));
        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.Number));
        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.Array));
        Assert.AreEqual(new Value(mapType), new Value(mapType).ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => new Value(mapType).ConvertTo(DataType.Undefined));
        ...
        Assert.Throws<InvalidCastException>(() => Value.Null.ConvertTo(DataType.Range));
        Assert.AreEqual(Value.Null, Value.Null.ConvertTo(DataType.Null));
        Assert.Throws<InvalidCastException>(() => Value.Null.ConvertTo(DataType.Undefined));

        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.Boolean));
        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.String));
        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.Number));
        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.Array));
        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.Map));
        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.Range));
        Assert.Throws<InvalidCastException>(() => Value.Undefined.ConvertTo(DataType.Null));
    ...
    {
        Assert.Throws<InvalidOperationException>(() => new Value(true).ValueAtIndex(new Value(0)));
        Assert.Throws<InvalidOperationException>(() => new Value(1).ValueAtIndex(new Value(0)));
        Assert.Throws<InvalidOperationException>(() => new Value().ValueAtIndex(new Value(0)));
        ...
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value(2)));
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value(-3)));
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value("asdf")));
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value("")));
        ...
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value(new[] { new Value(2) })));
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value(new[] { new Value(1), new Value(2) })));
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value(new[] { new Value("test") })));
        Assert.Throws<InvalidOperationException>(() => arrayValue.ValueAtIndex(new Value(new[] { new Value(0), new Value("test") })));
        ...
        Assert.Throws<InvalidCastException>(() => mapValue.ValueAtIndex(new Value(new[] { new Value(0) })));
        Assert.Throws<InvalidCastException>(() => mapValue.ValueAtIndex(new Value(new[] { new Value("test"), new Value(0) })));
    }
  ...
  spawnflags &  2 -> { ""path"": "":models/deadbods/dude/tris.md2"", ""frame"": 1 },
    ...
    {
        Assert.Throws<InvalidCastException>(() => Interpolate(" an \\${TEST} expression"));
    }
        
    [TestMethod]
    public void TestInterpolateStringWithUnterminatedEl()
    {
        Assert.Throws<TokenParsingException>(() => Interpolate(" an ${TEST"));
        Assert.Throws<TokenParsingException>(() => Interpolate(" an ${TEST expression"));
    }
